...
//this is an excessively long string I just so happened to have on hand, it's actually a NATO AmmoBearer's loadout from arma 3 which I ran through a few find & replaces in order to make it able to be received from a nodeJS server and then re-joined into a valid array for the command SetUnitLoadout in-game using a chain of splitstring/joins, I was going to do some testing in order to see if I could also upload a loadout from arma 3 to nodejs, but it turns out that the extension that I used only supported GET requests, and the one that purported to support any kind of request used a middleware to deserialize, but that middleware was on a glitch server, which the creator had made themself. Now, that would seemingly be fine, but it turns out that they took the deserialize project off of their glitch server. Meaning that it was, in effect, useless now.
